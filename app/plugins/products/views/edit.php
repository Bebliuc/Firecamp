<div class="content-box-header">	
		<h3>Editare produs: <span><?php echo $product->title; ?></span></h3>
		<ul class="content-box-tabs">
			<a href="#" class="right_button" id="adauga_produs" title="Adauga produs" style="margin-right:0px;">Salveaza produs</a> 
		</ul>
		<div class="clear"></div>		
</div>
<div class="content-box-content">
	<div id="upload_debug"></div>
	<form method="POST" action="<?php echo get_url('plugin/products/save/edit/'.$product->id); ?>" id="add_product_form" name="add_product_form" name="post">
		<div id="product_image">
			<img src="<?php echo BASE_URL; ?>public/products/90x90/<?php echo $product->image; ?>" title="Product Image"/>
		...
		<div id="product_promotion" class="<?php echo ($product->promotion == 1 ? 'on' : 'off'); ?>">
			<span class="promotion_button"><a href="#" title="Upload new image">Activare Promotie</a></span>
		</div>
		<fieldset>
			<p>
				<label for="title">Titlu produs</label>
				<input type="text" name="title" id="name" class="text-input medium-input" value="<?php echo $product->title; ?>" />
			</p>
			<p>
				...
				<select name="subcategory" id="subcategory" size="1" title="ajax">
					<?php foreach(Products::findSubcategories($product->category) as $subcategory):?>
						<option value="<?php echo $subcategory->id; ?>" <?php echo ($subcategory->id == $product->subcategory ? 'selected=""': ''); ?>>
            ...
            <p>
                ...
                <input type="text" id="product_support_file_value" name="file" value="<?php echo $product->file; ?>" class="text-input medium-input" />
            </p>
           	<p>
                <label for="products_manual_file_value">Manual file</label>
                <input type="text" id="product_manual_file_value" name="manual" value="<?php echo $product->manual; ?>" class="text-input medium-input" />
            </p>
			<p>
				<label for="description">Descriere produs</label>
				<textarea name="description" rows="5"><?php echo $product->description; ?></textarea>
			</p>
			<input type="hidden" id="product_image_value" name="image" value="<?php echo $product->image; ?>"/>
            <input type="hidden" id="product_promotion_image_value" value="<?php echo $product->image; ?>" name="promotion_image" />
			<input type="hidden" id="product_promotion_value" name="promotion" value="<?php echo $product->promotion; ?>" />
		</fieldset>
	</form>
</div>
