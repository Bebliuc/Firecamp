var QuploParser = Editor.Parser = (function() {
  ...
  function parseMixed(stream) {
    ...
    function top() {
      ...
      if (token.content == "<")
        inTag = true;
      else if (token.style.indexOf("xml-tagname") >= 0 && inTag === true)
        inTag = token.content.toLowerCase();
      else if (token.style.indexOf("xml-attribute") >= 0 && inTag == "page")
        pageType = token.content.toLowerCase().replace(/"(.*?)"/g,"$1");
        ...
        if (inTag == "script")
          ...
          if (pageType == "css")
            iter.next = local(CSSParser, "</page");
          else if (pageType == "js")
            iter.next = local(JSParser, "</page");
        }
        inTag = false;
        pageType = false;
      }
