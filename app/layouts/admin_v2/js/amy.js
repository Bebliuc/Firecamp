...
$class('ac.chap.theme.EAmy < ac.chap.Theme');
	...
	{
		$call(this, 'ac.chap.Theme.initDefinition');this.cssId = 'lazy';
this.background = '#fff';
this.textColor = '#000';
this.caretColor = '#000';
this.caretRowStyleActive = '#F3FFCA';
this.selectionStyle = '#DBFF85';
this.colorScheme[ac.chap.TOKEN_MULTIROW_COMMENT] = 'color:#8D868F';
this.colorScheme[ac.chap.TOKEN_SINGLEROW_COMMENT] = 'color:#8D868F';
this.colorScheme[ac.chap.TOKEN_SINGLE_QUOTED] = 'color:#109F00';
this.colorScheme[ac.chap.TOKEN_DOUBLE_QUOTED] = 'color:#109F00';
this.colorScheme[ac.chap.CHUNK_KEYWORD] = 'color:#FF7600';
this.colorScheme[ac.chap.CHUNK_NUMBER] = 'color:#3857B9';
this.colorScheme[ac.chap.CHUNK_OPERATOR] = 'color:#FF7600;';
this.colorScheme[ac.chap.CHUNK_PARENTHESIS] = 'color:#7500C0';
this.colorScheme[ac.chap.CHUNK_KEYWORD_CUSTOM] = 'color:#A90000';
this.colorScheme[ac.chap.CHUNK_FUNCTION_NAME] = 'color:#7500C0;font-weight:bold';
this.colorScheme[ac.chap.CHUNK_LIBRARY] = 'color:#7C8FAB';
this.colorScheme[ac.chap.CHUNK_LIBRARY_CUSTOM] = 'color:#3D4459';
}
...
+_+ "KEY: 101"
+_+ "	custom(action:'SmartUnindent', match_from:'start', starts_with:'if', ends_with:'else')"
+_+ "KEY: 125"
...
this.foldingStartMarkers = [/^.*\b(function|class)\s*(\w+\s*)?\([^\)]*\)(\s*\{[^\}]*)?\s*$/i];
...
this.chunkRules.push([/\b(break|case|continue|default|do|else|for|goto|if|_Pragma|return|switch|while)\b/i, 1, ac.chap.CHUNK_KEYWORD])
this.chunkRules.push([/\b(asm|__asm__|auto|bool|_Bool|char|_Complex|double|enum|float|_Imaginary|int|long|short|signed|struct|typedef|union|unsigned|void)\b/i, 1, ac.chap.CHUNK_LIBRARY])
this.chunkRules.push([/\b(const|extern|register|restrict|static|volatile|inline)\b/i, 1, ac.chap.CHUNK_LIBRARY])
this.chunkRules.push([/\b(break|c(ase|ontinue)|d(e(clare|fault)|ie|o)|e(lse(if)?|nd(declare|for(each)?|if|switch|while)|xit)|for(each)?|if|return|switch|use|while)\b/i, 1, ac.chap.CHUNK_KEYWORD])
this.chunkRules.push([/\b(catch|try|throw|exception)|([a-zA-Z_]*Exception)\b/i, 0, ac.chap.CHUNK_KEYWORD])
this.chunkRules.push([/\b(real|double|float|int(eger)?|bool(ean)?|string|class|clone|var|function|interface|parent|self|__(autoload|c(all|lone|onstruct)|destruct|get|set|sleep|toString|wakeup))\b/i, 1, ac.chap.CHUNK_LIBRARY])
this.chunkRules.push([/\b(global|abstract|const|extends|implements|final|p(r(ivate|otected)|ublic)|static)\b/i, 1, ac.chap.CHUNK_LIBRARY])
this.chunkRules.push([/&(?=\s*(\$|new|[A-Za-z_][A-Za-z_0-9]+(?=\s*\()))/i, 1, ac.chap.CHUNK_LIBRARY])
this.chunkRules.push([/(\$[A-Za-z_]*)/i, 1, ac.chap.CHUNK_LIBRARY_CUSTOM])
}
var snippet = {};
...
snippet = {tab_activation: '', code: ''};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'php', code: '?>$0<?php'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'echo', code: '<?php echo ${1:\$var} ?>$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'echoh', code: '<?php echo htmlentities(${1:\$var}, ENT_QUOTES, '+"'"+'utf-8'+"'"+') ?>$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'phperr', code: '`cat "$TM_BUNDLE_SUPPORT/textmate_error_handler.html" | grep -E -v '+"'"+'^[ \t\s]*\/\/'+"'"+' | sed '+"'"+'s/\(.*\)[ \t]*--.*/\1/'+"'"+' | tr -s '+"'"+' \n\t'+"'"+' '+"'"+' '+"'"+' | sed '+"'"+'s/.\{80\}[{};]/&\\n/g'+"'"+'`'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'foreach', code: '<?php foreach (\$${1:variable} as \$${2:key}${3: => \$${4:value}}): ?>\n	${0}\n<?php endforeach ?>'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'ifelse', code: '<?php if (${1:condition}): ?>\n	$2\n<?php else: ?>\n	$0\n<?php endif ?>'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'if', code: '<?php if (${1:condition}): ?>\n	$0\n<?php endif ?>'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'case', code: 'case '+"'"+'${1:variable}'+"'"+':\n	${0:# code...}\n	break;'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_c', code: '/**\n * ${3:undocumented class}\n *\n * @package ${4:default}\n * @author ${PHPDOC_AUTHOR:`niutil -readprop / "/users/$USER" realname`}\n **/\n$1class $2\n{$0\n} // END $1class $2'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'do', code: 'do {\n	${0:# code...}\n} while (${1:$a <= 10});'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'else', code: 'else {\n	${0:# code...}\n}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'elseif', code: 'elseif (${1:condition}) {\n	${0:# code...}\n}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'for', code: 'for (\$${1:i}=${2:0}; \$${1:i} < $3; \$${1:i}++) { \n	${0:# code...}\n}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'foreach', code: 'foreach (\$${1:variable} as \$${2:key}${3: => \$${4:value}}) {\n	${0:# code...}\n}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_f', code: '/**\n * ${4:undocumented function}\n *\n * @return ${5:void}\n * @author ${PHPDOC_AUTHOR:`niutil -readprop / "/users/$USER" realname`}$6\n **/\n$1function $2($3)\n{$0\n}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'if', code: 'if (${1:condition}) {\n	${0:# code...}\n}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'ifelse', code: 'if (${1:condition}) {\n	${2:# code...}\n} else {\n	${3:# code...}\n}\n$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'switch', code: 'switch (${1:variable}) {\n	case '+"'"+'${2:value}'+"'"+':\n		${3:# code...}\n		break;\n	$0\n	default:\n		${4:# code...}\n		break;\n}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'throw', code: 'throw new $1Exception(${2:"${3:Error Processing Request}"}${4:, ${5:1}});\n$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'while', code: 'while (${1:$a <= 10}) {\n	${0:# code...}\n}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'try', code: '${TM_SELECTED_TEXT/([\t ]*).*/$1/m}try {\n	${3:${TM_SELECTED_TEXT/(\A.*)|(.+)|\n\z/(?1:$0:(?2:\t$0))/g}}\n${TM_SELECTED_TEXT/([\t ]*).*/$1/m}} catch (${1:Exception} ${2:\$e}) {\n${TM_SELECTED_TEXT/([\t ]*).*/$1/m}	$0\n${TM_SELECTED_TEXT/([\t ]*).*/$1/m}}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'php', code: '?>$0<?php'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'echo', code: '<?php echo ${1:$var} ?>$0'};
...
snippet = {tab_activation: '', code: ''};
eamy.snippets.push(snippet);
snippet = {tab_activation: '$_', code: '\$_COOKIE['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: '$_', code: '\$_ENV['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: '$_', code: '\$_FILES['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: '$_', code: '\$_GET['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'globals', code: '\$GLOBALS['+"'"+'${1:variable}'+"'"+']${2: = }${3:something}${4:;}$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: '$_', code: '\$_POST['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: '$_', code: '\$_REQUEST['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: '$_', code: '\$_SERVER['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: '$_', code: '\$_SESSION['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'echoh', code: '<?php echo htmlentities(${1:$var}, ENT_QUOTES, '+"'"+'utf-8'+"'"+') ?>$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'phperr', code: '`cat "$TM_BUNDLE_SUPPORT/textmate_error_handler.html" | grep -E -v '+"'"+'^[ ts]*//'+"'"+' | sed '+"'"+'s/(.*)[ t]*--.*/1/'+"'"+' | tr -s '+"'"+' nt'+"'"+' '+"'"+' '+"'"+' | sed '+"'"+'s/.{80}[{};]/&\n/g'+"'"+'`'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'if', code: 'if (${1:condition}) {\n	${0:# code...}\n}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: '', code: ''};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_cp', code: '/**\n * ${1:undocumented class}\n *\n * @package ${2:default}\n * @author ${PHPDOC_AUTHOR:`niutil -readprop / "/users/$USER" realname`}$3\n **/$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_v', code: '/**\n * ${3:undocumented class variable}\n *\n * @var ${4:string}\n **/\n${1:var} \$$2;$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_vp', code: '/**\n * ${1:undocumented class variable}\n *\n * @var ${2:string}$3\n **/$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_d', code: '/**\n * ${3:undocumented constant}\n **/\ndefine($1, $2);$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_dp', code: '/**\n * ${1:undocumented constant}$2\n **/$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_fp', code: '/**\n * ${1:undocumented function}\n *\n * @return ${2:void}\n * @author ${PHPDOC_AUTHOR:`niutil -readprop / "/users/$USER" realname`}$3\n **/$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_s', code: '/**\n * ${4:undocumented function}\n *\n * @return ${5:void}\n * @author ${PHPDOC_AUTHOR:`niutil -readprop / "/users/$USER" realname`}$6\n **/\n$1function $2($3);$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_h', code: '/**\n * $1\n *\n * @author ${PHPDOC_AUTHOR:`niutil -readprop / "/users/$USER" realname`}\n * @version \$Id\$\n * @copyright `echo $TM_ORGANIZATION_NAME`, `date +"%e %B, %Y"`\n * @package ${3:default}\n **/\n\n/**\n * Define DocBlock\n **/\n'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_i', code: '/**\n * ${2:undocumented class}\n *\n * @package ${3:default}\n * @author ${PHPDOC_AUTHOR:`niutil -readprop / "/users/$USER" realname`}\n **/\ninterface $1\n{$0\n} // END interface $1'};
eamy.snippets.push(snippet);
snippet = {tab_activation: '/**', code: '/**\n * $0\n */'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'foreach', code: '<?php foreach ($${1:variable} as $${2:key}${3: => $${4:value}}): ?>\n	${0}\n<?php endforeach ?>'};
...
snippet = {tab_activation: '', code: ''};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'ret', code: 'return$1;$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'ret0', code: 'return false;$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'ret1', code: 'return true;$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'if', code: '<?php if (${1:condition}): ?>\n	$0\n<?php endif ?>'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'case', code: 'case '+"'"+'${1:variable}'+"'"+':\n	${0:# code...}\n	break;'};
...
snippet = {tab_activation: 'while', code: 'while (${1:$a <= 10}) {\n	${0:# code...}\n}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'try', code: '${TM_SELECTED_TEXT/([t ]*).*/$1/m}try {\n	${3:${TM_SELECTED_TEXT/(A.*)|(.+)|nz/(?1:$0:(?2:t$0))/g}}\n${TM_SELECTED_TEXT/([t ]*).*/$1/m}} catch (${1:Exception} ${2:$e}) {\n${TM_SELECTED_TEXT/([t ]*).*/$1/m}	$0\n${TM_SELECTED_TEXT/([t ]*).*/$1/m}}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: '$_', code: '$_COOKIE['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: '$_', code: '$_ENV['+"'"+'${1:variable}'+"'"+']'};
...
snippet = {tab_activation: 'doc_dp', code: '/**\n * ${1:undocumented constant}$2\n **/$0'};
eamy.snippets.push(snippet);
snippet = {key_activation:'68',tab_activation: '', code: '/**\n * ${1:undocumented function}\n *\n * @return ${2:void}\n * @author ${PHPDOC_AUTHOR:`niutil -readprop / "/users/$USER" realname`}$3\n **/$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_s', code: '/**\n * ${4:undocumented function}\n *\n * @return ${5:void}\n * @author ${PHPDOC_AUTHOR:`niutil -readprop / "/users/$USER" realname`}$6\n **/\n$1function $2($3);$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'doc_h', code: '/**\n * $1\n *\n * @author ${PHPDOC_AUTHOR:`niutil -readprop / "/users/$USER" realname`}\n * @version $Id$\n * @copyright `echo $TM_ORGANIZATION_NAME`, `date +"%e %B, %Y"`\n * @package ${3:default}\n **/\n\n/**\n * Define DocBlock\n **/\n'};
...
snippet = {tab_activation: 'ret1', code: 'return true;$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'def', code: 'define('+"'"+'$1'+"'"+', ${2:'+"'"+'$3'+"'"+'});\n$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'array', code: '$${1:arrayName} = array('+"'"+'$2'+"'"+' => $3${4:,} $0);'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'incl', code: 'include '+"'"+'${1:file}'+"'"+';$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'def?', code: '$1defined('+"'"+'$2'+"'"+')$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'echo', code: 'echo "${1:string}"${0};'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'req', code: 'require '+"'"+'${1:file}'+"'"+';$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'incl1', code: 'include_once '+"'"+'${1:file}'+"'"+';$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'tmphp', code: '// === TextMate error handling ===\n`if [[ "$TM_BUNDLE_SUPPORT" == "$HOME"* ]]; then\n  echo "// NOTE: Your PHP bundle is checked out to your home directory."\n  echo "// If the webserver process does not have permission to access"\n  echo "// the included file, you can replace"\n  echo "// â€˜${TM_BUNDLE_SUPPORT%Bundles*}â€™ with"\n  echo "// â€˜$(find_app TextMate.app)/Contents/SharedSupport/â€™."\nfi`\n@include_once '+"'"+'$TM_BUNDLE_SUPPORT/textmate.php'+"'"+';\n'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'req1', code: 'require_once '+"'"+'${1:file}'+"'"+';$0'};
eamy.snippets.push(snippet);
	...
	// style.setAttribute('href', 'http://www.amyeditor.com/api/embed/amy_style.php');
