	...
	$.fn.iToggle = function(options) {
		...
		this.each(function(){
			...
			if($this.attr('tagName') == 'INPUT'){
				...
				$this.addClass('iT_checkbox').before('<label class="itoggle" for="'+id+'"><span title="' + title +'"></span></label>');
...
$j(function(){
	...
	$j('#change-branch').click(function(event){
		event.preventDefault();
		
		$j('#change-branch, #submit-branch').toggleMe();
		
		$j('#activity-selector').find('span, form').toggleMe().end().find('select').focus().change(function(){
	...
	$j("#dropables div#dropable div.content").hide();
	
	//$j('#dropables div#dropable div.content').css('height', '0');
	$j('#dropables div#dropable h2').click(function(e) {
		var $parent = $j(this).parent().children('div.content');
		var $height = $parent.height();
		
		if($j(this).attr('class') != 'expanded') {
			$parent.animate({ 'height' : 'toggle', 'opacity' : '1'}, 200);
			$j(this).addClass('expanded');
		}
		else {
			$parent.animate({ 'height' : 'toggle', 'opacity' : '0.25'}, 200);
